use crate::instruction::BridgeInstruction;
use borsh::{BorshDeserialize, BorshSerialize};
use solana_program::{
    account_info::{next_account_info, AccountInfo},
    entrypoint::ProgramResult,
    msg,
    program_error::ProgramError,
    pubkey::Pubkey,
};

pub struct Processor {}
impl Processor {
    pub fn process(
        program_id: &Pubkey,
        accounts: &[AccountInfo],
        instruction_data: &[u8],
    ) -> ProgramResult {
        let instruction = BridgeInstruction::try_from_slice(instruction_data)
            .map_err(|_| ProgramError::InvalidInstructionData)?;
        match instruction {
            BridgeInstruction::Construct => {}
            BridgeInstruction::TransferRequest => {}
            BridgeInstruction::TransferReceipt => {}
            BridgeInstruction::UpdateVerifyAddressOnlyOwner => {}
            BridgeInstruction::UpdateTokenLimitOnlyOwner => {}
            BridgeInstruction::SetTokenLimitTimeOnlyOwner => {}
            BridgeInstruction::UpdateStableFeeOnlyOwner => {}
            BridgeInstruction::UpdateTokenFeeOnlyOwner => {}
            BridgeInstruction::UpdateFees => {}
            BridgeInstruction::WithdrawFeesOnlyOwner => {}
            BridgeInstruction::AddTokenOnlyOwner => {}
            BridgeInstruction::PauseTokenOnlyOwner => {}
            BridgeInstruction::UnpauseTokenOnlyOwner => {}
            BridgeInstruction::CalculateFee => {}
            BridgeInstruction::Verify => {}
        }
        Ok(())
    }
}
